...
export class OwnedRoomPragma extends Pragma {
  ...
  spawn() {
    ...
    let T2_carry = this.assigned.filter(a => {
      let c = byId(a);
      if (c != undefined) {
        return c.memory.role === RoleTypes.T2_Carry;
      }
      return;
    });
    let T2_work = this.assigned.filter(a => {
      let c = byId(a);
      if (c != undefined) {
        return c.memory.role === RoleTypes.T2_Worker;
      }
      return;
    });
    let container = this.getSourceContainer();

    if (T2_carry.length <= 2 && container != undefined && container.store.energy > 50) {
      spawnRole(
        this.OwnedRoom,
        this.id,
        RoleTypes.T2_Carry,
    ...
    let T1_worker = this.assigned.filter(a => {
      let c = byId(a);
      if (c != undefined) {
        return c.memory.role === RoleTypes.T1;
      }
      return;
    });
    if (!(T2_work.length < 1)) maxCreeps = 1;
    if (
      this.getSourceContainer != undefined &&
      Game.rooms[this.OwnedRoom].energyAvailable >= 500 &&
      T2_work.length < 1
    ) {
      spawnRole(
        this.OwnedRoom,
    ...
    } else if (T1_worker.length < maxCreeps && this.getSourceContainer != undefined) {
      spawnRole(this.OwnedRoom, this.id, RoleTypes.T1, roles[RoleTypes.T1](Game.rooms[this.OwnedRoom].energyAvailable));
    }
  }
  action(creep: Creep) {
    if (!this.checkOwnedRoom() || this.disabled) return;
  ...
  private getSourceContainer() {
    let container = Memory.OwnedRooms[this.OwnedRoom].sources.find(a => {
      return a.id == this.sourceId;
    })?.container;
