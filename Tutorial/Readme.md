...
So far we have been spawning creeps directly from the console, we will need to automate this to allow for the colony to support itself. We need to create new creeps when old creeps die, as there are no events when creeps die the best way to do this is to count the required creeps like so:
```js
var roleHarvester = require('role.harvester');
var roleUpgrader = require('role.upgrader');
...
The above code will output the number of creeps with the "Harvester" role into the console. We want to modify our code to automatically have two harvesters at a time, we will spawn a harvester every time the number of harvesters are less than two:
```js
var roleHarvester = require('role.harvester');
var roleUpgrader = require('role.upgrader');
